...
func (handler *streamHandler) responseStream(ctx context.Context) {
	...
	for activeStreaming {
		...
		if err == nil {
			...
			case !grpcStatus:
				handler.receiver.logger.Warn("response stream breaking on error",
					zap.Error(err))
				activeStreaming = false
			case status.Code() == codes.Unavailable:
				handler.receiver.logger.Info("response stream breaking on gRPC status")
				activeStreaming = false
			case status.Code() == codes.NotFound:
				handler.receiver.logger.Error("resource doesn't exist, wait 60 seconds, and restarting stream")
				time.Sleep(time.Millisecond * 60)
				activeStreaming = false
			default:
				handler.receiver.logger.Warn("response stream breaking on gRPC status",
		...
		if ctx.Err() == context.Canceled {
			...
			handler.receiver.logger.Warn("response stream ctx.Err() == context.Canceled")
			break
		}
	}
