...
func TestStartReceiver(t *testing.T) {
	...
	receiver := &pubsubReceiver{
		...
		config: &Config{
			...
			Traces: TracesConfig{Subscription: "my-traces"},
			Metrics: MetricsConfig{Subscription: "my-metrics"},
			Logs: LogsConfig{Subscription: "my-logs"},
		},
...
func TestStartReceiverWithNoServer(t *testing.T) {
	...
	receiver := &pubsubReceiver{
		...
		config: &Config{
			...
			Traces: TracesConfig{Subscription: "my-trace-topic"},
			Metrics: MetricsConfig{Subscription: "my-metric-topic"},
			Logs: LogsConfig{Subscription: "my-log-topic"},
		},
		tracesConsumer:  consumertest.NewTracesNop(),
		metricsConsumer: consumertest.NewMetricsNop(),
		logsConsumer:    consumertest.NewLogsNop(),
...
func TestStartReceiverNoSubscription(t *testing.T) {
	...
	receiver := &pubsubReceiver{
		...
		config: &Config{
			...
			Traces: TracesConfig{Subscription: "no-trace"},
			Metrics: MetricsConfig{Subscription: "no-metric"},
			Logs: LogsConfig{Subscription: "no-log"},
		},
	}
