...
func (c postgreSQLConfig) ConnectionString() (string, error) {
	...
	if c.address.Transport == "unix" {
		// expects a unix socket host to start with a "/" and appends the appropriate .s.PGSQL.port internally
...
func newPostgreSQLClient(db *sql.DB) *postgreSQLClient {
	return &postgreSQLClient{client: db}
}
