...
| ce-time | a watermark indicating when the events, encapsulated in the OTLP message, where generated. The behaviour will depend on the watermark setting in the configuration |
...
attributes are useful for real time streaming analytics, de-duplication or anomaly detection. The behaviour of the 
...
The **default** behaviour is that the watermark is set to the current time of the processor. This timestamp will not differ
...
feature you may want to change the behaviour of the watermark and de-duplication. You can set the attributes names on
...
scenario is `behaviour: earliest` with a reasonable `allow_drift` of `1h`.

Allowed behaviour values are `current` or `earliest`. For `allow_drift` the default is `0s`, so make sure to set the 
value.