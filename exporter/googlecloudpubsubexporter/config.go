...
type Config struct {
	...
	// Watermark defines the watermark (the ce-time attribute on the message) behaviour
	Watermark WatermarkConfig `mapstructure:"watermark"`
}

// WatermarkConfig customizes the behaviour of the watermark
type WatermarkConfig struct {
	// Behaviour of the watermark. Currently, only  of the message (none, latest, earliest and current, current being the default)
...
func (config *WatermarkConfig) validate() error {
	_, err := config.parseWatermarkBehaviour()
	if err != nil {
		return err
	}
	return nil
...
func (config *WatermarkConfig) parseWatermarkBehaviour() (WatermarkBehavior,error) {
	...
	return Current, fmt.Errorf("if specified, behaviour should be earliest or current ")
}
